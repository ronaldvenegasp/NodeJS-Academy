...
const formatCount = (count) => {
  if (count) {
    ...
    const [integerPart, decimalPart] = newCount
      .toString()
      .split(".")
      .map((element) => parseInt(element, 10));

    if (!decimalPart) return newCount;

    if (integerPart === 0) {
      const fr = new Fraction(newCount);
      return `${fr.numerator}/${fr.denominator}`;
    } else {
      const fr = new Fraction(newCount - integerPart);
      return `${integerPart} ${fr.numerator}/${fr.denominator}`;
    }
  }
  return "?";
};
